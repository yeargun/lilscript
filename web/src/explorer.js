...
const rateLabel = (baseline, result) => {
  ...
  return rate >= 0
    ? `${percent(rate)} smaller`
    : `${percent(Math.abs(rate))} larger`;
};
const rows = catalog.projects.flatMap((project, projectIndex) =>
...
controls.category.insertAdjacentHTML(
  "beforeend",
  values(({ project }) => project.category)
    .map((value) => option(value, labels[value] ?? value))
    .join(""),
);
controls.status.insertAdjacentHTML(
  "beforeend",
  values(({ project }) => project.status)
    .map((value) => option(value))
    .join(""),
);
controls.tool.insertAdjacentHTML(
  "beforeend",
  values(({ artifact }) => artifact.tool)
    .map((value) => option(value))
    .join(""),
);
controls.mode.insertAdjacentHTML(
  "beforeend",
  values(({ artifact }) => artifact.mode)
    .map((value) => option(value))
    .join(""),
);
...
for (const [key, control] of Object.entries({
  ...
  const value = queryState.get(key);
  if (
    value &&
    [...control.options].some((candidate) => candidate.value === value)
  )
    control.value = value;
}
if (queryState.get("q")) controls.search.value = queryState.get("q");

const eligible = catalog.projects.filter(
  (project) => project.status === "eligible",
).length;
document.querySelector("[data-catalog-summary]").innerHTML = [
  ...
  .join("");
document.querySelector("[data-definitions]").innerHTML = Object.entries(
  catalog.definitions,
)
  .map(
    ([term, definition]) =>
      `<article><h3>${escape(term)}</h3><p>${escape(definition)}</p></article>`,
  )
  .join("");
...
function haystack(row) {
  return [
    ...
    row.project.id,
    row.project.summary,
    ...(row.project.exclusions ?? []),
    row.project.category,
    row.project.status,
    row.artifact.label,
    row.artifact.tool,
    ...row.project.packages.map((item) => item.name),
  ]
    .join(" ")
    .toLowerCase();
}
function compare(left, right) {
  ...
  if (key === "core") {
    ...
    a = left.artifact[key];
    b = right.artifact[key];
  } else if (key === "gzip-rate") {
    a = percentageSaved(left.artifact.raw, left.artifact.gzip);
    b = percentageSaved(right.artifact.raw, right.artifact.gzip);
  } else if (key === "brotli-rate") {
    a = percentageSaved(left.artifact.raw, left.artifact.brotli);
    b = percentageSaved(right.artifact.raw, right.artifact.brotli);
  } else if (key === "tool") {
    a = left.artifact.tool;
    b = right.artifact.tool;
  } else if (key === "category") {
    a = left.project.category;
    b = right.project.category;
  } else {
    a = left.project.title;
    b = right.project.title;
  }
  const order =
    typeof a === "number" ? a - b : String(a).localeCompare(String(b));
  return (
    (ascending ? 1 : -1) *
    (order || left.artifact.label.localeCompare(right.artifact.label))
  );
}
function renderAggregates(filtered) {
  ...
  const cards = summary
    ? [
        [
          percent(summary.meanBrotliReduction),
          "average saved by Brotli-11",
          "Mean of each lane's raw-to-Brotli reduction",
        ],
        [
          percent(summary.meanBrotliEdge),
          "average Brotli edge",
          "Mean reduction from gzip bytes to Brotli bytes",
        ],
        [
          percent(summary.meanGzipReduction),
          "average saved by gzip-9",
          "Mean of each lane's raw-to-gzip reduction",
        ],
        [
          formatBytes(summary.meanRaw),
          "mean raw output",
          "Arithmetic mean before transport compression",
        ],
      ]
    : [
  ...
  document.querySelector("[data-aggregate-summary]").innerHTML = cards
    .map(
      ([value, label, detail]) =>
        `<article><strong>${escape(value)}</strong><span>${escape(label)}</span><small>${escape(detail)}</small></article>`,
    )
    .join("");
...
function render() {
  document
    ...
    .classList.toggle("show-all-columns", controls.view.value === "full");
    ...
    .filter(
      (row) =>
        ...
        (!controls.category.value ||
          row.project.category === controls.category.value) &&
        (!controls.status.value ||
          row.project.status === controls.status.value) &&
  ...
  const state = {
    ...
    view: controls.view.value === "core" ? "" : controls.view.value,
    sort: controls.sort.value === "core" ? "" : controls.sort.value,
  };
  ...
  document.querySelector("[data-result-count]").textContent =
    `${number.format(filtered.length)} of ${number.format(rows.length)} artifact rows across ${new Set(filtered.map((row) => row.project.key)).size} projects`;
  document.querySelector("[data-explorer-rows]").innerHTML =
    filtered
      .map(
        ({ project, artifact }) => `
    <tr>
      ...
      .join("") ||
    '<tr><td colspan="10" class="empty-table">No artifact matches these filters.</td></tr>';
  renderIcons(document.querySelector("[data-explorer-rows]"));
}
for (const control of Object.values(controls).filter(
  (control) => control !== controls.direction,
))
