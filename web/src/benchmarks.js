...
document.querySelector("[data-paired-method]").textContent =
  `Generated ${pairedData.generatedAt.slice(0, 10)} from one neutral workload schema. Every row passed Closure JavaScript, LilScript JavaScript, emitted C, and native execution before its per-case ${pairedData.costModel ?? "brotli"} size gate; raw and gzip remain visible. ${browserData.browser} then measured 50 warmed alternating samples; the runtime column is the bootstrap upper confidence ratio, gated at ${browserData.regressionLimit.toFixed(2)}x.`;
