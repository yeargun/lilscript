...
use crate::semantic::{BuiltinCall, NominalId, NominalMemberId, SymbolId, Type};
use crate::span::Span;
use std::sync::Arc;
...
impl UnitData {
    ...
    ) -> Option<&'types crate::semantic::FunctionParameter<'src>> {
...
/// directory, as the legacy linker spells it; a bare specifier is kept.
#[derive(Debug, Clone)]
...
impl<'src> Program<'src> {
    ...
    pub fn parameter(&self, cell: CellId) -> Option<&crate::semantic::FunctionParameter<'src>> {
    ...
    pub fn to_javascript(&self) -> Result<crate::structured_js::Module, Unsupported> {
...
/// an ordinary class may dissolve, as on the legacy route. An `extern class` names a
