...
use crate::semantic::{
    CheckedModules, ClassInfo, ExpressionResolution, NominalMember, SemanticModel, SemanticView,
};
...
/// Unsupported contracts are visible; no legacy linker/emitter is invoked.
...
fn interface_target(target: crate::semantic::InterfaceTarget) -> Option<InterfaceTarget> {
    Some(match target {
        crate::semantic::InterfaceTarget::Value(symbol) => {
            InterfaceTarget::Value(CellId::from_index(symbol.0 as usize)?)
        }
        crate::semantic::InterfaceTarget::Struct(identity) => InterfaceTarget::Struct(identity),
    })
}

struct Lower<'budget, 'ledger, 'sem, 'ast, 'src> {
    semantics: SemanticView<'sem, 'ast, 'src>,
...
impl<'sem, 'ast, 'src> Lower<'_, '_, 'sem, 'ast, 'src> {
    ...
    fn ty(&mut self, ty: &Type<'src>) -> Result<TypeId, ConversionError> {
        ...
        use crate::semantic::type_payload::{
        ...
        for (index, known) in self.program.types.iter().enumerate() {
            if crate::semantic::type_relation::type_equal_with(
                known,
                ty,
    ...
    fn register_class(
        ...
        for member in declaration.members {
            ...
            params.push(crate::semantic::FunctionParameter::value(receiver));
            ...
            let function = crate::semantic::FunctionType::new(crate::semantic::FunctionSignature {
                params,
                return_type,
            });
            ...
            let callable = self.ty(&if info.type_params.is_empty() {
                ...
                Type::GenericFunction(crate::semantic::GenericFunctionType {
    ...
    fn class_instantiation(
        ...
        while class != method.class {
            ...
            for argument in &base_arguments {
                substituted.push(
                    crate::semantic::type_substitution::substitute_type_with(
                        argument,
                        &mut |name: &str, _: &mut crate::semantic::type_relation::Unmetered| {
                            Ok(parameters
                                .iter()
                                .position(|parameter| *parameter == name)
                                .and_then(|index| arguments.get(index)))
                        },
                        &mut crate::semantic::type_relation::Unmetered,
                    )
        ...
        let effective = crate::semantic::type_substitution::substitute_type_with(
    ...
    fn parameters_with_receiver(
        ...
        for (index, parameter) in parameters.iter().enumerate() {
            ...
            if matches!(default, crate::semantic::DefaultValue::Undefined) {
            ...
            let value = match default {
                crate::semantic::DefaultValue::Parameter(earlier) => {
    ...
    fn array_destructure(
        ...
        for (position, binding) in bindings.iter().enumerate() {
            ...
            let item = if matches!(binding, ast::ArrayBinding::Rest(_)) {
                let signature = self.ty(&Type::Function(crate::semantic::FunctionType::new(
                    crate::semantic::FunctionSignature {
                        params: vec![
                            crate::semantic::FunctionParameter::defaulted(
                                Type::Int,
                                crate::semantic::DefaultValue::Int(0),
                            ),
                            crate::semantic::FunctionParameter::defaulted(
                                Type::Int,
                                crate::semantic::DefaultValue::Int(i32::MAX as i64),
                            ),
                        ],
    ...
    fn materialize_defaults(
        ...
        while end > values.len()
            && matches!(
                ...
                Some(crate::semantic::DefaultValue::Arrow(_))
            )
        {
            end -= 1;
        }
        for position in values.len()..end {
            ...
            if matches!(default, crate::semantic::DefaultValue::Arrow(_)) {
    ...
    fn default_value(
        ...
        unit: UnitId,
        region: RegionId,
        default: &crate::semantic::DefaultValue<'src>,
        ty: TypeId,
        values: &[CallArgument],
        callee: bool,
        span: Span,
    ) -> Result<ValueId, ConversionError> {
        use crate::semantic::DefaultValue;
