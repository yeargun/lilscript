...
mod scratch;
use crate::semantic::binary_types::{checked_binary_type_with, BinaryTypeError};
use crate::semantic::type_admission::TypeQueryAdmission;
use crate::semantic::type_relation::{
    is_type_assignable_with as structurally_assignable, type_equal_with, RelationAdmission, RelationEvent,
};
use crate::semantic::type_substitution::substitute_signature_with;
...
fn validate_signature(
    signature: &crate::semantic::FunctionSignature<'_>,
    query: &mut TypeQueryAdmission<'_, '_>,
...
fn verify_type_contracts(
    ...
    use crate::semantic::type_payload::{measure_payload, Payload, PayloadError};
...
fn callable_signature<'a, 'src>(
    ...
    unit: &UnitData,
) -> Result<Option<&'a crate::semantic::FunctionSignature<'src>>, VerificationError> {
    match (
        unit.kind,
        unit.callable_type
            .and_then(|id| program.types.get(id.index())),
...
fn verify_types(
    ...
    match &operation.kind {
        ...
        OperationKind::Template => {
            ...
            for &value in operands {
                if !crate::semantic::binary_types::is_stringable_with(value_type(value), &mut query)? {
                    return Err(error());
                }
            }
            Ok(())
        ...
        OperationKind::Call(call) => {
            ...
            let argument_matches = |expected: &crate::semantic::FunctionParameter<'_>,
                                    actual: &CallArgument,
                                    query: &mut TypeQueryAdmission<'_, '_>|
             -> Result<bool, VerificationError> {
            ...
            let full_arity = |signature: &crate::semantic::FunctionType<'_>,
                              query: &mut TypeQueryAdmission<'_, '_>|
             -> Result<bool, VerificationError> {
...
fn materialized_default(
    unit: &UnitData,
    default: &crate::semantic::DefaultValue<'_>,
    value: ValueId,
            ...
            || matches!(actual, Type::ClassInstance { .. }) =>
        {
            ...
            loop {
                ...
                for &argument in &class.base_arguments {
                    next.push(crate::semantic::type_substitution::substitute_type_with(
