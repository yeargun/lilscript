...
use crate::semantic_program::uses::PreparedUseUpdate;
...
pub(in crate::semantic_program) struct FixedUnitEdits<'view, 'src> {
...
impl<'view, 'src> FixedUnitEdits<'view, 'src> {
    ...
    pub(in crate::semantic_program) fn program(&self) -> &'view Program<'src> {
        self.program
    }
    pub(in crate::semantic_program) fn previous_uses(&self) -> &'view UseIndex {
        self.previous_uses
    }
    pub(in crate::semantic_program) fn changes(&self) -> &'view [UnitChange] {
        self.changes
    }
}
