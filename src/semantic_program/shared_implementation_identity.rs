...
use super::*;
use crate::semantic_program::rewrite_lineage::RewriteLineage;
use std::sync::Arc;
...
pub(in crate::semantic_program) struct SharedImplementationIdentity(Arc<SharedIdentity>);

impl SharedImplementationIdentity {
    pub(in crate::semantic_program) fn ensure(
    ...
    pub(in crate::semantic_program) fn build(
        map: Option<&ImplementationMap>,
        snapshot: RevisionId,
        meaning: RevisionId,
        lineage: &RewriteLineage,
    ...
    pub(in crate::semantic_program) fn meaning(&self) -> RevisionId {
        self.0.meaning
    }
    pub(in crate::semantic_program) fn snapshot(&self) -> RevisionId {
        self.0.snapshot
    }
    pub(in crate::semantic_program) fn tactics(&self) -> &[crate::compilation_policy::TacticUse] {
        self.0.lineage.tactics()
    }

    pub(in crate::semantic_program) fn owner(&self) -> RevisionId {
        self.0.owner
    }

    pub(in crate::semantic_program) fn fingerprint(&self) -> u64 {
        self.0.fingerprint
    }

    pub(in crate::semantic_program) fn description(&self) -> ImplementationDescription<'_> {
    ...
    #[cfg(test)]
    pub(in crate::semantic_program) fn words(&self) -> &[u32] {
        self.0.identity.words()
    }

    pub(in crate::semantic_program) fn equivalent(
        &self,
        other: &Self,
        budget: &mut AllocationBudget<'_>,
    ...
    pub(in crate::semantic_program) fn compare(
        &self,
        other: &Self,
        budget: &mut AllocationBudget<'_>,
