...
impl ResolvedPolicy {
    ...
    pub fn receipt(&self) -> serde_json::Value {
        ...
        let objective = self.objective.map(|o| json!({"codec":format!("{:?}",o.codec), "priority":format!("{:?}",o.rank.priority), "realistic_performance_limit_percent":o.rank.realistic_performance_limit_percent, "optional_alternatives":o.optional_alternatives, "optional_codec_probes":o.optional_codec_probes, "cleanup_finalists":o.cleanup_finalists, "retained_candidates":o.retained_candidates, "retained_candidate_bytes":o.retained_candidate_bytes, "beam_width":o.beam_width, "search":{"version":SEARCH_SCHEDULE_VERSION,"codec_schedule":o.search.codec_schedule,"render_batch":o.search.render_batch,"diversity_interval":o.search.diversity_interval}}));
