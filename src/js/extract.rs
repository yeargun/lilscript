...
impl<'a> Output<'a> {
    ...
    pub(crate) fn render_bundle_with_literals_admitted<Owner: Eq + Copy>(
            ...
            .zip(dynamic)
        {
            if entry.is_none() {
                ...
                // reuse, as the old route did; other modes do not.
    ...
    /// The old route's split rule on this partition. A module only
...
impl Delivered {
    /// The old route's bundle deploy cost: every file's weighted codec
