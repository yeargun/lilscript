//! Native run-time strings. Literals stay static; every string made while the
//! program runs lives until execution ends, which is the lifetime callback
//! ABI v1 already promises hosts for every string payload. `main` frees them
//! all when it returns. A slice or trim is a view of its source, since both
//! are immutable and live equally long.
//!
//! Conversions follow ECMA-262: Number::toString picks the shortest decimal
//! that reads back as the same binary64, and `print` writes a float the way
//! `console.log` does, which spells negative zero `-0`. Text is written as
//! UTF-8 with lone surrogates replaced by U+FFFD, as Node writes it.
//!
//! Case mapping covers ASCII text. Other text stops the program rather than
//! map it differently from JavaScript; full Unicode tables are recorded work.

pub(super) const STRINGS: &str = r#"#include <stdlib.h>
