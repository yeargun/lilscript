...
fn cyclic_live_import_and_reexport_are_the_original_mutable_symbol() {
    ...
    let program = crate::semantic_program::from_checked_modules(&programs, &checked).unwrap();
...
fn module_checking_admits_parameter_defaults_for_caller_and_callee_evaluation() {
    // The semantic route evaluates an omitted default at each typed call site
