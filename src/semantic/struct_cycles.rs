...
use super::{SemanticError, StructInfo, Type};
...
) -> Result<(), (Option<crate::module::ModuleId>, SemanticError)> {
    ...
    for root in 0..structs.len() {
        ...
        while let Some(frame) = path.last_mut() {
                    ...
                    .is_none_or(|schema| schema.declaration.identity != declaration.identity)
            {
                return Err((
                    owner.module,
                    SemanticError::new(field.span, "invalid canonical struct field declaration"),
                ));
            }
            match visits[next] {
                Visit::Active => {
