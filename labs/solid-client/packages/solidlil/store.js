/* solidlil/store — Solid-compatible fine-grained stores on the LilScript runtime */
import {
  batch,
  $PROXY,
  $TRACK,
...
import { hasListener } from "./reactive.generated.js";

const $RAW = Symbol("store-raw"),
  $NODE = Symbol("store-node"),
  $HAS = Symbol("store-has"),
  $SELF = Symbol("store-self");
const storeDevHooks = { onStoreNodeUpdate: null };
...
function getNode(nodes, property, value) {
  ...
  const [s, set] = createSignal(
    value,
    CORE_DEV ? { equals: false, internal: true } : { equals: false },
  );
  s.$ = set;
...
function trackSelf(target) {
  hasListener() && getNode(getNodes(target, $NODE), $SELF)();
}
...
const proxyTraps$1 = {
  ...
  has(target, property) {
    if (
      ...
      return true;
    hasListener() && getNode(getNodes(target, $HAS), property)();
...
function setProperty(state, property, value, deleting = false) {
  ...
  const prev = state[property],
    len = state.length;
  CORE_DEV?.hooks &&
    storeDevHooks.onStoreNodeUpdate?.(state, property, value, prev);
  if (value === undefined) {
...
function updatePath(current, path, traversed = []) {
  ...
  if (path.length > 1) {
    ...
    if (
      partType === "string" &&
      (part === "__proto__" || (path.length > 1 && isUnsafeKey(part)))
    ) {
      return;
    }
...
function createStore(...[store, options]) {
  ...
  CORE_DEV?.registerGraph({ value: unwrappedStore, name: options?.name });
...
const DEV = CORE_DEV ? { $NODE, isWrappable, hooks: storeDevHooks } : undefined;
