/* solidlil/web — SolidJS web DOM mechanics, reactive core from LilScript */
import {
  createMemo,
  sharedConfig,
  createRenderEffect,
...
// The 46-export client distribution has no hydration entrypoint. Its build
// defines this flag so hydration-only control flow and helpers disappear from
// the final tree-shaken chunk. The complete browser entry leaves it false.
const CLIENT_ONLY =
...
const ChildProperties = /*#__PURE__*/ new Set(
  "innerHTML textContent innerText children".split(" "),
);
...
const DelegatedEvents = /*#__PURE__*/ new Set(
  "beforeinput click dblclick contextmenu focusin focusout input keydown keyup mousedown mousemove mouseout mouseover mouseup pointerdown pointermove pointerout pointerover pointerup touchend touchmove touchstart".split(
    " ",
  ),
);
...
// A Set discards the duplicate entries present in Solid's source table. Keep
// the same first-occurrence iteration order without paying to construct those
// entries in every reusable browser bundle.
const DOMElements = /*#__PURE__*/ new Set(
  "html base head link meta style title body address article aside footer header main nav section blockquote dd div dl dt figcaption figure hr li ol p pre ul a abbr b bdi bdo br cite code data dfn em i kbd mark q rp rt ruby s samp small span strong sub sup time u var wbr area audio img map track video embed iframe object param picture portal source svg math canvas noscript script del ins caption col colgroup table tbody td tfoot th thead tr button datalist fieldset form input label legend meter optgroup option output progress select textarea details dialog menu summary slot template acronym applet basefont bgsound big blink center content dir font frame frameset hgroup image keygen marquee menuitem nobr noembed noframes plaintext rb rtc shadow spacer strike tt xmp h1 h2 h3 h4 h5 h6 webview isindex listing multicol nextid noindex search".split(
    " ",
  ),
);
...
function eventHandler(e) {
  if (!CLIENT_ONLY && sharedConfig.registry && sharedConfig.events) {
  ...
  if (!CLIENT_ONLY && sharedConfig.registry && !sharedConfig.done)
...
function NoHydration(props) {
  return !CLIENT_ONLY && sharedConfig.context ? undefined : props.children;
}
...
function innerHTML(parent, content) {
  (CLIENT_ONLY || !sharedConfig.context) && (parent.innerHTML = content);
}
...
function createElement(tagName, isSVG, is) {
  return isSVG
...
function Portal(props) {
  const { useShadow } = props,
    ...
    owner = getOwner();
  let content;
  let hydrating = !CLIENT_ONLY && !!sharedConfig.context;
  createEffect(
    () => {
  ...
  return createMemo(() => {
    ...
    switch (typeof component) {
      ...
      case "string":
        ...
        const el =
          !CLIENT_ONLY && sharedConfig.context
            ? getNextElement()
            : createElement(
                component,
                isSvg,
