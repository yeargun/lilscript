...
> History: describes the compiler route deleted on 2026-09-23 (plan M1); kept as prior art.

Status: descriptive comparison, not a normative pass contract. Current behavior
is source/tests and [current architecture](compilation/current-architecture.md).

Reasoning for codec-scored search and pass intent: [knowledge/compilation](../compilation/README.md). Closure comparison evidence: [knowledge/evidence/closure-comparison.md](../evidence/closure-comparison.md).
...
| Numeric representation lowering                     | Signed-i32 range analysis propagates bounded loop induction values, direct-call arguments/returns, and owned nominal fields. Compiler-generated redundant normalization may be dropped by proof/policy; a live source-authored `value \| 0` carries an explicit lowering obligation. Ordinary multiplication emits `x*y \| 0` when i32 normalization is required, while source `Math.imul` remains an exact explicit operation. The slower `-~x` increment spelling is not emitted. |
...
| Inline functions and simple methods                 | Fixed-point expression inlining plus single-use multi-block CFG inlining; size-first single-file/ESM builds compare that IR with a fully outlined IR under the exact selected codec. A separate script-only emission proof substitutes private pure return-helper DAGs with stable/eager-single-evaluation actual bindings, comparing named, single-static-use, and all-eligible policies as complete artifacts                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Inline/collapse properties                          | Nominal field resolution, positional field indexes, struct/class scalar replacement (including an atomic, closed-use decomposition of structured-loop aggregate phis into field phis), and owned-field range/finite-constant summaries invalidated at untyped boundaries                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Alias strings and numbers                           | Repeated constants are value-numbered; profitable long strings and numeric literals receive shared short bindings, including explicitly declared/imported pools in split ESM chunks. Size-first also considers delimiter-packed immutable string tables and proof-gated complete lookup tables for zero-based bounded integer ladders returning strings; final pooling, packing, table, helper-policy, quote, and coercion variants are selected against the exact configured raw, gzip, or Brotli cost                                                                                                                                                                                                                                                                                                                                                 |
...
coalescing regression extracted from the Solid client-runtime gate. The final
JavaScript peephole also refuses to reuse a binding captured by an escaped
closure; that regression was caught when a closed-world SolidLil disposer lost
its owner identity and failed to unmount the application.
...
the Brotli-target enabled artifact is required to beat its disabled ablation in
Brotli-11. Raw and gzip-9 are reported diagnostics and may lose. The current
checked workload measures `216/155/118` bytes versus `143/108/77`.

The focused optimizer benchmarks that follow share that pass-isolation scope:
each run gates only the metric selected by its named config. Other codec sizes
describe the artifact but do not establish wins in those objectives.
...
raw/gzip/Brotli. Both artifacts must produce `137`; all three sizes are
reported, but only the selected Brotli metric is a non-regression gate. The complete
MurmurHash port is also byte-identical between modes at `1734/831/733`; neither
workload is presented as a loop-spelling win.
...
workloads, runs all outputs, invokes Closure `ADVANCED`, and builds independent
raw-, gzip-, and Brotli-selected LilScript artifacts. Each artifact is gated
only on its matching exact byte metric. The benchmark is a reproducible corpus result,
...
generated C, and native execution. Its reusable raw-, gzip-, and Brotli-objective
modules also pass an API-shape and behavior checksum before their matching size
cells are published; the Brotli artifact supplies the declared runtime lane.
The root-owned SolidLil lab separately verifies the
pinned browser Core, Web, and Store contract: 135/135 public exports and 469/469
unchanged upstream tests. Its JavaScript-specific compiler corpus contributes
112 cases across maximum and disabled optimization (224 executions), while
lifecycle gates cover stale handles, collection teardown, late resources, and
stable owner/effect pools. The LSX frontend remains a separate incomplete gate
and is never counted as runtime parity evidence.
