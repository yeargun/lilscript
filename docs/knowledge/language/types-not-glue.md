...
| `extern class C` | host ABI names by default; explicit closed-world config may mangle; never `new` | existing host object | rejected |
...
**`int` vs `number`.** Bitwise/shift stay `int`-only because JS itself i32-coerces them. Ordinary `int` multiply is `(a*b)|0`, never silently `Math.imul`. Source `Math.imul` stays exact low-32. Size-first and balanced may drop compiler-generated proven-redundant normalization; a live source-written `value | 0` remains explicit. `performance-first` and `javascript.integer_coercions = true` retain generated normalization too.
...
backing after observation projection.
