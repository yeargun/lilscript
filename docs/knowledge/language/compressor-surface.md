...
whether the syntax and proofs were enough — not whether the compiler needs
another fold.
...
|---|---|
| `struct` / `class` with closed fields | scalar-replace local allocations; positional arrays; rename owned names (plan M9.6) |
...
| `pure` / inferred effects | delete unused calls (from plan M7.2) |
| static `import` | one program across files; no ESM wrappers in the artifact |
| `extern` / `extern class` only at the host | exact ABI; everything else dissolves |
| `object O { … }` for a closed singleton API | ABI keys; private bodies nest/mangle (does not compile today; plan M10.10) |
...
| `JS.method*` / `defineProperty` constructor tables | identity reconstructed in user space; the compiler must then recover the `class` |
| `record{}` when the JS contract is ordinary `{}` | `Record<T>` materializes null-prototype; jQuery observes `Object.prototype` |
| `createEmptyObject()` as the default object | host trampoline; an extern means nothing by its name, so the compiler knows nothing about it |
...
Public names are contract (the target and declared boundaries, D2), not an
...
valid scalar-replacement on/off ablation. The deleted route had a scored
`keep-object` IR clone for that question
([history](../history/compilation/decision-registry.md#aggregates-class-struct-object-record));
on this compiler, layout becomes one codec-judged choice (plan M9.7), which
isolates the representation question directly.
...
| PostHog error-tracking | old-style constructable classes via `JS.method*` + `defineProperty` | migrate to the constructor-value export (refused until M4.1) and a named `class` where API-equivalent |
...
expression form the language never had; the deleted route's
`local_phi_expression_regions` recovery lost under Brotli and was off by default.
...
| `object` singleton | ABI keys, private nestable bodies (does not compile today; M10.10) | rebuilt as `JS.object()` + `JS.method*` |
| `class` methods / `this` | typed receiver, static dispatch | use `export constructor` only when the constructor value is public |
| `pure` / inferred effects | unused calls removable (from M7.2) | `JsValue` ops are effectful; purity cannot override that |
...
JavaScript bindings. Monaco, Motion, Zod, and Redux depend on that. Flipping
`export class` to emit a constructor would reintroduce `constructor` /
`prototype` on identity-free types the ports deleted on purpose.
...
|---|---|---|
| **Constructor value** distinct from type-only `export class` (landed on the deleted route; refused until M4.1) | `export constructor C [as PublicC];` publishes the constructor-value contract and drives named class emission. | Emitting `class` for identity-free cases; changing the type-only default |
...
| **Getters / setters as ABI** | `defineProperty` / Proxy traps when the published contract is an accessor | The compiler inventing accessors on dissolved fields |
...
| **Application/library ABI contract** (partial) | The contract and objective exist; the old source-derived manifest went with the deleted route. Independent world/format axes (M3.1) and a parse of every delivered file (M2.5) remain. | Disabling internal optimization in library mode; letting raw/gzip/Brotli alter the public API |
...
because the deleted route's SSA coalescing and name-keyed host lowering
...
3. the language is missing a proof — use the reusable-proof migration, cases first, no
   library-specific fold.
