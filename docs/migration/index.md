...
**Implementation status: 0/14 milestones verified.** 001–010 are implemented, 011–013 are active on the reference ports and 014 waits. None is verified. No independent review has run, and the ledger's accepted receipts for 003–007 pin source files that have changed since: `node finer/tools/migration-progress.mjs` reported 46 findings on 2026-09-22. Receipts are refreshed once, on the binary that closes a milestone, not after every batch. Existing mechanisms and historical results can be requalified, but are not completion receipts for this plan.
...
**Compute (owner, 2026-09-22).** Every build, test and measurement runs on this host. There is no worker pool and none is to be recreated; `finer/tools/workers.mjs` is retired for this plan. The semantic route makes this affordable: the whole fleet built and ran its own suites here in 504 s on 2026-09-21 ([007e](../../benchmarks/migration-results/2026-09-21-semantic-ports-007e/stdout.txt)). The inner loop is probelil. markedlil, katexlil and zodlil (plus jquerylil's suite) gate each batch. The fleet runs at milestone closure, and whenever a default changes, since a default is judged on the fleet average. The host is burstable, so timings come from alternating pairs within one session, never from rows taken hours apart.

**No accepted losses (owner, 2026-09-22).** No library may end as a recorded loss; "almost tie" is the floor and a win the target. Diagnose a loss on both sides: the port's `.lil` (typing, `pure`, value structs and the language's other capabilities, so the compiler has room) and the compiler (finer, generic heuristics). A port rewrite is recorded as `finer/port-migrations/<port>.patch`, or as a branch in the port's repository once it outgrows a patch. The port's own suites are the gate. The comparison target stays the competitor's build of upstream.
...
The [2026-09-22 architecture audit](../../benchmarks/migration-results/2026-09-22-architecture-audit/README.md) replaces the 2026-09-19 source inspection. Its measurements, on this host with commit `577d472d`:

| Evidence | Consequence |
|---|---|
| Both routes are in one binary. `[compiler] backend` or `--backend` selects one, and the default is still `legacy` | 014 retires the default route once 011–013 close; nothing ships through the semantic route before that |
| With the search off on both routes, the semantic compiler is 3–12× faster and smaller on the three module ports (markedlil 9,397 against 10,179, zodlil 28,326 against 33,995, katexlil 56,132 against 59,999). The default route's production builds take 50–100× longer because its search re-emits the whole program from IR 267–381 times, at 0.16–1.8 s each. That search buys it 4–7% | 012 states speed at equal effort. The architecture's real saving is per candidate, and the semantic route leaves almost all of that compile-time budget unused |
| The semantic route renders a katexlil candidate in about 14 ms. Six final Brotli-11 encodes are 70% of its 3.5 s, and its search tries six candidates because no family proposes alternatives | Search cost is now codec cost. 010's scheduler needs real alternatives, and the default route's 44 scored emission families are the inventory to port as checked target choices (013-T4) |
| Every byte gained in 009–013 comes from rewrites of the finished JS tree: forwarding, inlining, store folds, namespace flattening and spellings. The typed families (scalar replacement, leaf-helper inlining, constant folding, specialization, pooling) add 0–48 bytes. Terser applied to our output makes markedlil and zodlil larger, so no minifier slack is left there | The "better typed, so better optimized" payoff has not been realized. Further bytes must come from what only the typed program knows, and those facts must reach the edits that change output (013-T1–T3) |
| probelil, the typed feature probe, is 1,836 (as a module) against the default route's 1,492. The script-frame rule accounts for 27 bytes. The rest is the default route's IR inlining (+271 when switched off), call-site specialization, positional structs and int-typed reads without `\|0` | The typed optimizations the default route performs are the first bar the semantic route must clear, before any competitor |
| The whole fleet built and ran its suites on the semantic route here in 504 s (007e, 2026-09-21) | Fleet suites and cells run on this host. The worker-pool dependency in 011–013 is removed |
| `migration-progress.mjs` reports 46 findings: stale 003–007 receipts, and six progress rows with undeclared states | States are corrected in the table below. Receipts are refreshed on the binary that closes each milestone |
| [`maintained-workloads.json`](../../benchmarks/libraries/maintained-workloads.json) has 27 libraries plus Probe and related `lil-solidjs` coverage | Preserve all 29 identities and roles, and their scoped required-case inventories |
...
| [ ] | [011 Public compiler and fleet integration](#011-public-compiler-and-fleet-integration) | 007-010 | active; `[compiler] backend` selects the route explicitly, and the backend and policy are recorded on every build. Reference-port suites pass through public entrypoints: katexlil, zodlil, markedlil (29/29 with a representation patch) and jquerylil. Compression losses recorded for 013. The fleet's suites run on this host at closure (007e ran all 25 here in 504 s). | [011 status](#011-status-2026-09-22-route-selection-reference-suites-through-public-entrypoints) |
| [ ] | [012 Compilation speed and resource gates](#012-compilation-speed-and-resource-gates) | 011 | active; with the search off on both routes the semantic compiler is 3–12× faster (katexlil 1.0 against 4.3 s), at a quarter or less of the peak RSS. The default route's 27–352 s production builds are its own search. Runtime is equal on markedlil and ~10% slower on katexlil. Envelopes are not yet frozen, p95 is not measured, and the fleet has not run. | [012 status](#012-status-2026-09-22-faster-per-candidate-and-the-saving-is-not-yet-spent) |
| [ ] | [013 Compression qualification](#013-compression-qualification) | 011, 012 | active; katexlil loses to Terser by 2,683 Brotli on the semantic route, narrowed from 4,017 by two batches (the default route loses 1,576). No loss is accepted (owner, 2026-09-22): child tasks 013-T1–T6. zodlil and markedlil need comparable competitor boundaries (013-T5). The fleet cells have not run. | [013 status](#013-status-2026-09-22-katex-is-a-loss-the-other-reference-boundaries-are-not-comparable) |
...
**What keeps the gate open.** The full-fleet suites have not run on the current binary. There is no worker pool (owner, 2026-09-22), and none is needed: `node finer/tools/semantic-port-tests.mjs --compiler <binary> --ports <all> --out <report>` ran all 25 ports' suites on this host in 504 s for 007e, when six ports still had failing cases. The fleet run belongs at 011's closure, on the binary that closes it.
...
### 012 status (2026-09-22): faster per candidate, and the saving is not yet spent
...
**Correction (architecture audit, same day).** The table compares a searching route with a non-searching one. At equal effort, with the search off on both routes ([timing matrix](../../benchmarks/migration-results/2026-09-22-architecture-audit/README.md#1-compile-time-faster-by-architecture-but-the-saving-is-not-being-reinvested)), the semantic compiler takes 0.02–1.0 s against the default route's 0.25–4.3 s, and its output is smaller on the three module ports. The two orders of magnitude are the default route's search: every candidate re-emits the whole program from IR, 1.8 s each on markedlil, and 267 of them take 93% of its CPU. The semantic route renders a candidate in about 14 ms. Its production time is mostly six final Brotli-11 encodes (2.48 of katexlil's 3.5 s), and it explores six candidates. So the architecture's saving is per candidate, and it is unspent: the same 264-candidate search would cost about 20 s of CPU on markedlil against 506 s. How that budget is spent belongs to 010 and 013-T4. Codec cost now decides it, so ranking by cheap estimates and scoring only finalists with Brotli-11 comes first.
...
### 013 child tasks (2026-09-22): the typed program reaches the output, and the search gets choices

The [architecture audit](../../benchmarks/migration-results/2026-09-22-architecture-audit/README.md) groups every open loss under two causes:
- **Lost facts.** Formation hands the target an untyped tree, and the edits that change output re-derive legality from JS syntax. The facts the program keeps are too weak: calls are `UNKNOWN`, field and parameter loads are never typed domains, there are no ranges, and `pure` is dropped.
- **Unavailable choices.** Every target spelling is applied or withheld globally, so the search, which can now afford hundreds of candidates, has six to score.

The tasks below fix general mechanisms. Each names its owner, its reproduction, the negative cases that must keep passing, and its cost limit. Every batch runs the unit suite, the census (72/72/72) and probelil's two lanes. markedlil, katexlil, zodlil and jquerylil gate each batch. The fleet runs at closure.

| ID | Task | Owner | Reproduction | Negative cases | Cost limit |
|---|---|---|---|---|---|
| 013-T1 | **Facts cross formation.** One whole-program summary, computed once in `SemanticProgram` and keyed by target identities, answers what the target edits now guess: value domain (int32, number, string, bool, non-null, function) from typed producers, never from the declaration alone at a public boundary; per-function effects (reads, writes, throws, may diverge, runs user code), and `pure` as a checked contract; root initialization order. The four effect models and two TDZ owners collapse into it. The target edits (`inline.rs`, `mod.rs` forwarding and flattening) consult it, and `Expr::Host` spelling stops deciding pristine globals | `semantic_program/facts.rs`, formation, `structured_js/inline.rs` | katexlil `Ma` (a root constant read after root calls); katexlil `buildCommon` (the 68-byte TDZ guard); zodlil `(b.optin\|0)==2`; `(a.width\|0)` on probelil; forwarding refused across a checked `a+b` | A coercing getter or `valueOf` reached through a field; a public entry passing a string to an `int` parameter; a diverging pure call whose result is unused (D3.6); reads before initialization (D3.7); sloppy-script frames (D3.9) | Linear in program size; no rescan per candidate; compile time on katexlil within 10% |
| 013-T2 | **Whole-program inlining and specialization on checked facts.** The default route's IR inliner is worth 271 / 42 / 521 / 645 bytes (probelil, markedlil, zodlil, katexlil) at level 0. Once-called functions inline as blocks, never as IIFEs (Closure's `FunctionInjector` block mode), and calls with constant arguments specialize. Admission uses T1's effects instead of syntactic inertness | `structured_js/inline.rs` on T1's summary | probelil `f(a)` with defaults 10 and 100 (the default route prints `a+110`); katexlil `sqrtPath`, `tallDelim` and the SVG path builders (Terser's `unused` + `reduce_vars` leave-one-out, +1,245 / +1,150); markedlil `j(d,b,c)` inside a sequence | Recursion (D3.10); arguments with effects evaluated once and in order; `this`/`arguments` bodies; exported or reassigned functions; frames observable in scripts | No closure allocation added on a hot path; katexlil and markedlil runtime within noise |
| 013-T3 | **Typed layouts and names.** Private classes and value structs become scalars where they do not escape, and positional storage where they do, as a scored choice. Int-typed field and element reads drop `\|0` when T1 proves every producer int32. Closed-world field renaming comes back for typed fields (markedlil's closed lane) | product family, `from_source.rs` class lowering, naming | probelil `Shape`/`Labelled`/`Holder` (`{width:0,height:0}` with setter functions); markedlil's closed lane (renamed option keys); zodlil's 254 `\|0` against the default route's 26 | Identity-observing classes (`instanceof`, host subclasses, D2 boundaries); fields a host or `JsValue` path can read; generic structs | Removing every `\|0` measures only −4 to −79 Brotli, so T3 is judged on layouts and names, not on `\|0` |
| 013-T4 | **Choices for the search.** The default route's 44 scored emission families are the inventory. Port them as checked target alternatives that one scheduler scores, and rank by cheap estimates so that only finalists get Brotli-11. Start with regex literals (markedlil −85, zodlil −18, katexlil +24: a choice, not a rule), then conditional expressions, update and compound assignment, declaration merging, loop spelling and literal pooling | `structured_js` spellings, `semantic_program/search*.rs` | The 118 `new RegExp("…")` on markedlil; the 2–3× statement density against the default route | Every alternative is a checked target rewrite with a written legality condition; printed choices live on the tree, never in thread-local policy | ≤ 10% of the default route's wall time on each reference port |
| 013-T5 | **Comparable boundaries.** zodlil publishes 2 names against upstream's 240, and markedlil 8 against 18. Build each competitor from upstream restricted to the port's declared public names, with the same externals, or publish the port's full surface. "Not comparable" cannot close a cell | `finer/tools/competitor-recipes.mjs` | zodlil and markedlil in [013 status](#013-status-2026-09-22-katex-is-a-loss-the-other-reference-boundaries-are-not-comparable) | The restricted competitor must still pass the port's API tests for those names | Minutes per port, on this host |
| 013-T6 | **katexlil through the language.** Retype the port module by module, so T1–T3 have facts to use: numbers, strings, structs, enums, non-null values and `pure`. Today its 90 source files carry 3,990 `JsValue` mentions, 394 `toNum` calls and no `pure`. Measure each module's Brotli against the same slice of Terser's build of upstream | the port, through [katexlil.patch](../../finer/port-migrations/katexlil.patch) or a port branch | katexlil 65,727 against Terser's 63,044 | katex's own suites (21/21, 1,230/1,230) | Module by module, each landing only with the suites green |
