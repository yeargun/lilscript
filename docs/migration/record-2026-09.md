# Migration record, 2026-09-19 to 2026-09-23 (milestones 001-013)

**This is a history record, not a plan.** On 2026-09-23 the owner ruled that there is one compiler, and the plan was rewritten around it: [index.md](index.md) is the only plan and [future-architecture.md](../future-architecture.md) the architecture. This file keeps the evidence, batch ledger, measurements and 013-T7 inventory that the new plan cites. In it, "semantic route" names the compiler that remains and "default route" or "legacy route" names the one being deleted.

---

# (Former title) Compiler Migration Plan
...
| Concern | Owner |
|---|---|
| Checked meaning | `Program` in [semantic_program/mod.rs](../../src/program/mod.rs), built by [from_source.rs](../../src/program/from_source.rs) |
...
| Snapshots, edits, checked rewrites, lineage | `Compilation` in [publication.rs](../../src/program/publication.rs), [publication_edits.rs](../../src/program/publication_edits.rs), [publication_rewrites.rs](../../src/program/publication_rewrites.rs), [rewrite_lineage.rs](../../src/program/rewrite_lineage.rs) |
| Facts and uses | [facts.rs](../../src/program/facts.rs) (session, cache, billing) and [uses.rs](../../src/program/uses.rs) |
| What the artifact must compute | `DemandPlan` in [demand.rs](../../src/program/demand.rs) |
| Representation families | [record_family.rs](../../src/program/record_family.rs), [string_family.rs](../../src/program/string_family.rs), [product_family.rs](../../src/program/product_family.rs) with [function_layout.rs](../../src/program/function_layout.rs), [helper_family.rs](../../src/program/helper_family.rs), [value_placement.rs](../../src/program/value_placement.rs) |
| JavaScript formation and public adapters | `Formation` in [javascript.rs](../../src/program/javascript.rs), [javascript_structs.rs](../../src/program/javascript_structs.rs), [javascript_public_structs.rs](../../src/program/javascript_public_structs.rs), [javascript_struct_boundaries.rs](../../src/program/javascript_struct_boundaries.rs) |
| Target syntax, names, printing, verification | [structured_js](../../src/js/mod.rs): `naming.rs`, `print.rs`, `verify.rs`, `extract.rs` |
| Artifacts, admission, provenance | `ArtifactArena` in [artifacts.rs](../../src/program/artifacts.rs), [artifact_provenance.rs](../../src/program/artifact_provenance.rs) |
| Search | [search.rs](../../src/program/search.rs), [search_selection.rs](../../src/program/search_selection.rs), [search_entries.rs](../../src/program/search_entries.rs) |
| Native | [native.rs](../../src/program/native.rs), [native_plan.rs](../../src/program/native_plan.rs), [artifact_native.rs](../../src/program/artifact_native.rs) |
| Public entry | `compile_source_semantic`/`compile_path_semantic` in [compiler_service.rs](../../src/build.rs); `lilscript --backend semantic` |
...
Contracts: A2-A7. Consume 006's interfaces and 008's target/delivery owner. Use [optimization coverage](../knowledge/history/optimization-coverage.md) as inventory and the design's competitor mapping as questions to test, not parity evidence.
...
Terser's `reduce_vars` + `unused` subset alone takes 807 Brotli bytes off our katexlil output. Most of it resolves reads of never-mutated namespace objects: the transliteration's `__star` modules, read as `html.buildGroup(…)`. Two generic target edits cover the sound part ([inline.rs](../../src/js/inline.rs)):
...
- **Exact operator folds** ([simplify.rs](../../src/js/simplify.rs)), after literal folding and again at the end of target compaction. Each rule keeps operands, evaluation order and effects.
...
- **Block inlining (013-T2's first slice).** `inline_single_calls` in [blocks.rs](../../src/js/blocks.rs) handles `let f=(p)=>{…}` called once, as `f(a);`, `let x=f(a);`, `x=f(a);` or `return f(a);`. The call becomes `{let p=a;…}`, which is Closure's `FunctionInjector` block mode without its labels. The conditions:
...
- **Repeated strings are root constants, and string arrays are split strings.** [pooling.rs](../../src/js/pooling.rs), last in formation once no edit reads a literal:
...
- **Initializers become their stores.** [initializers.rs](../../src/js/initializers.rs) runs under pristine builtins, before the store fold. It rewrites a call `f(o,v,…);` into `o.k=v;…` when:
...
- **Statements as expressions** ([statements.rs](../../src/js/statements.rs)), in the raw objective's structure, bottom-up until nothing applies:
...
- **Settled reads** ([mod.rs](../../src/js/mod.rs) `forward_single_uses`). A value of literals, functions and binding reads, or arrays and objects of them, moves to the first later statement that mentions its binding, as inert values already did. That needs three conditions:
...
- **Raw objective**, in [statements.rs](../../src/js/statements.rs):
...
- **A quiet start** ([quiet.rs](../../src/js/quiet.rs)). A value may now move to its read in the next statement when everything evaluated before that read is quiet. A quiet evaluation runs no code, cannot throw, and yields a value the moved value cannot change. Quiet evaluations are:
...
- **Raw objective** ([statements.rs](../../src/js/statements.rs)):
...
The rule is now per site ([inline.rs](../../src/js/inline.rs), with `expression_owners` and `runs_after_root` in [quiet.rs](../../src/js/quiet.rs)):
...
- **A temporary tested once is the logical operator.** The ports transliterate `a&&b`, `a||b` and `a??b` as a temporary and a test. katexlil has 24 of the `&&` shape and 64 of `||`, jquerylil 19 of `||` and 7 of `??`, zodlil 14 of `??`. `fold_logical_assignments` ([statements.rs](../../src/js/statements.rs)) prints these as the operator:
...
- **An uninitialized declaration joins the one before it**, as with Terser's `join_vars`: `let a=1;S;let b` becomes `let a=1,b;S` when nothing in `S` names `b`. It looks back at most 16 statements and stays within one source module ([declarations.rs](../../src/js/declarations.rs)).
...
Three changes:
- **A function with one call is created at it** (`place_single_calls`, [blocks.rs](../../src/js/blocks.rs)). `let f=(p)=>{…};…f(a)` becomes `…((p)=>{…})(a)`. Creating a function runs no code, so it may happen where the call is, under these conditions:
...
- **A trailing argument that repeats the callee's default goes** (`drop_default_arguments`, [mod.rs](../../src/js/mod.rs)). Formation passes a source call's omitted defaults explicitly: `buildFragment(elems, ctx, scripts)` printed as `zb(k,f,j,null,null)`. When the callee is never reassigned, reads no `arguments` object, and its body opens with `if(p===void 0)p=D` for that literal, the argument goes. So does a trailing `undefined`.
...
- **Initializer stores the literal already holds go** (`drop_redundant_init_stores`, [initializers.rs](../../src/js/initializers.rs)).
...
- **A default only an erased caller could use is no check** (`drop_typed_default_checks`, [mod.rs](../../src/js/mod.rs)). Typed callers always pass a value for a parameter whose type excludes `undefined`. Formation records such parameters (`Module.defined_parameters`), the first type fact crossing into the tree. A function whose every use is a direct call loses those opening checks. Keeping a check when some call passes the default literal (so that call could drop the argument) measured worse on motionlil (+159), so the checks go.
- **Initializer folding is per site** ([initializers.rs](../../src/js/initializers.rs)). One construction in an expression position no longer disables folding for every construction of the class, and an argument for a typed parameter counts as defined. motion's color constructions now print as literals, `a=>({red:fc(a,"red",0),…})`.
- **An object only read through its fields is its fields** (`scalarize_member_objects`, [scalar_objects.rs](../../src/js/scalar_objects.rs)).
...
- **A null test of an object is its truthiness** (`truthy_null_tests`, [typed.rs](../../src/js/typed.rs)). An object is never falsy. For a binding that holds an object or null, `x!=null` in a test position (an `if` or loop condition, a conditional's test, a `!` operand, and through `&&`/`||` there) is `x`, and `x==null` is `!x`.
- **An array the program created takes its own methods** (`array_receiver_calls`, [typed.rs](../../src/js/typed.rs)). `JS.push(a, v)` lowers to `Array.prototype.push.call(a,v)`, since a `JsValue` may be anything. When every value a local binding ever receives is an array literal, and builtins are pristine, the call is `a.push(v)`. The same holds for `pop`, `slice`, `indexOf`, `sort`, `splice`, `join`, `shift` and `unshift`.
- **A number counts with `++`** ([print.rs](../../src/js/print.rs)). `x=x+1` of a binding that always holds a number prints `++x`: the same store and the same value. An int's wrapping `x=x+1|0` is another expression and keeps its spelling.
...
- **`x.m.call(x,…)` is `x.m(…)`** (`self_method_calls`, [calls.rs](../../src/js/calls.rs)).
...
- **A lambda that ignores its receiver needs no receiver adapter** (`dissolve_receiver_adapters`, [calls.rs](../../src/js/calls.rs)).
...
- **Literal root constants are their literal** (`forward_root_constants`, [root_constants.rs](../../src/js/root_constants.rs); 013-T7.4; Closure's `InlineVariables`).
  - The rule: every read of a never-assigned root `let c=L` of a literal becomes `L`, wherever `c` is initialized.
  - Initialization order decides that, and it is now sharper than creation order ([quiet.rs](../../src/js/quiet.rs)): a function created by root statement `c` cannot run before the first root statement from `c` on that may run code. Every quiet-prefix and forwarding rule reads the same fact.
...
- **Sharing a repeated literal is a separate choice** ([pooling.rs](../../src/js/pooling.rs); Closure's `AliasStrings`). It is made once for all literals, by the objective. Under the raw objective the pool names repeated strings and now repeated numbers (`281474976710655` read four times is one name). A codec objective keeps the repeated text it matches.
- **Nullish narrowing** ([simplify.rs](../../src/js/simplify.rs)). `x==null&&x!==void 0` is `x===null`, and `x!=null||x===void 0` is `x!==null` (and with `null` and `void 0` swapped). This is the port's `isNull`, `JS.isNullish(v) && !JS.isUndefined(v)`, which the inliner could not take while it read its parameter twice.
- **`Object.prototype.hasOwnProperty.call(o,k)` is a boolean** to the operator rules, so `!!` around it goes.
- **The quote that escapes least** ([print.rs](../../src/js/print.rs)). A string holding more `"` than `'` prints in single quotes. micromark's entity table, a JSON text, had 8,520 escaped quotes.
- **A host value assumed to be a struct is decoded by name** (`assumed_product`, [javascript.rs](../../src/program/javascript.rs)).
...
- **An operator on a primitive literal is inert** ([mod.rs](../../src/js/mod.rs)). `-5`, `!0` and `typeof "a"` run no code. A table of negative codes was kept alive by that one gap: its statement counted as running code, which also blocked every constant declared after it. Root constants are forwarded again after the store folds, once `JS.set` sequences have become literals.
- **The quote choice is the raw spelling's** ([print.rs](../../src/js/print.rs)). The raw objective prints each string in the quote it escapes least. A codec keeps one delimiter: requoting react-markdown's entity table saved 7,008 raw bytes and cost 53 Brotli, and katexlil's few switched strings cost +80.
...
A measurement by a parallel session (splice experiments on b70 artifacts, `~/lilscript-work/data-encoding/`) found that re-encoding large constant tables removes novelty every codec pays for. Closure, Terser, Oxc and esbuild re-encode no data. This batch lands the string-table family (`encode_string_tables`, [tables.rs](../../src/js/tables.rs)):
...
- **The rule now** ([calls.rs](../../src/js/calls.rs)):
...
The markdown ports still named some of micromark's token types (`M="lineEnding"`) after batch 26. Three gaps in [quiet.rs](../../src/js/quiet.rs) and the declaration pruning caused it:
