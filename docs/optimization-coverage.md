...
Reasoning for codec-scored search and pass intent: [knowledge/compilation](knowledge/compilation/README.md). Closure comparison evidence: [knowledge/evidence/closure-comparison.md](knowledge/evidence/closure-comparison.md).
...
| Numeric representation lowering                     | Signed-i32 range analysis propagates bounded loop induction values, direct-call arguments and returns, and owned nominal fields; it drops proven `|0` for size-first and balanced because `|0` never helps gzip/Brotli (`performance-first` and `javascript.integer_coercions = true` keep it), including remainder results whose nonzero divisor was pooled behind a numeric alias. Ordinary multiplication emits `x*y                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 0`when normalization is required and source-written`Math.imul`remains an explicit exact operation. The formerly compact`-~x`increment spelling is deliberately not emitted because Chromium 151 measured a material runtime regression; ordinary`+1` retained identical i32 semantics and whole-artifact transfer size in the checked paired case |
