...
2. **Phase 6 under that regime**: the residue after 7.25 is `fold_int32_coercions` (G6, 468 on
   the cases), `fold_unit_counter_updates` and `fold_self_assignment_chains` on the probe's
   closures, `fold_single_use_*`/`fold_identity_arrow_iife` (G4/G5 as the single-use collapse on
   the tree with the fact word, the measured byte lever, +280/+296 on micromark), the declaration
   folds (`merge_adjacent_declarations`, `remove_unused_standalone_vars`), G8, the scored groups
   as `ShapeTransform`s, G12 last. With the search on the text layer is two thirds of jquerylil's
   CPU (7.25): 7e (candidates keyed on plan identity) lets 7a's per-emission fold ship.
