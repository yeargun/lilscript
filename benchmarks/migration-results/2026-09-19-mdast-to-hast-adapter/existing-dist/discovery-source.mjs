...
const scratch = mkdtempSync(join(tmpdir(), "lilscript-mdast-discovery-20260919-"));
...
const files = ["test/site.test.mjs", "test/to-hast.test.mjs", "test/official/index.js"];
const fixtures = [
  "package.json", "package-lock.json", "tsconfig.json", "types/to-hast.d.ts", "test/types.d.ts", "test/types.test.ts",
  "scripts/check-pack.mjs", "scripts/build-site.mjs", "site/index.html", "site/app.js", "site/results.json",
  ...readdirSync(join(original, "test/official")).filter(name => name !== "index.js").sort().map(name => `test/official/${name}`),
].map(local);
const prerequisites = [{
  id: "original-test-types", executable: identify(npm), args: ["run", "test:types"], timeoutMs: 60_000,
  inputs: ["package.json", "package-lock.json", "tsconfig.json", "types/to-hast.d.ts", "dist/to-hast.d.ts", "test/types.d.ts", "test/types.test.ts", "node_modules/typescript/package.json", "node_modules/typescript/bin/tsc", "node_modules/typescript/lib/tsc.js"].map(local),
}];
const started = process.hrtime.bigint();
const elapsedMs = () => Number(process.hrtime.bigint() - started) / 1e6;
const receipt = {
  schema: 1, kind: "existing-dist-mdast-to-hast-inventory-discovery", started: new Date().toISOString(),
  ...
  limitations: [
    ...
    "The original upstream package supplies only numeric footnote helper reference values; all candidate toHast calls use the generated ESM/closed files.",
    "Original Node selection does not execute CJS/UMD, install the package, rebuild the site or exercise a browser.",
...
try {
  ...
  const report = await runNodeTestEvidence({
    cwd: original, files, requiredFilePatterns: ["test/*.test.mjs", "test/official/index.js"],
    artifactPaths: ["dist/to-hast.esm.js", "dist/to-hast.closed.js"],
