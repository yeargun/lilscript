...
import { copyFileSync, cpSync, existsSync, globSync, mkdirSync, mkdtempSync, readFileSync, readdirSync, realpathSync, symlinkSync, writeFileSync } from "node:fs"
...
export function sanitizeEnvironment(env, { node, scratch, userConfig, globalConfig, compiler }) {
  ...
  // Some ports compile a fixture inside their own suite and find the compiler
  // through a path relative to their workspace. A scratch copy has no such
  // sibling, so name the binary explicitly and pin its identity in the receipt
  // instead of letting the suite reach for whatever happens to be on disk.
  if (compiler) overrides.LILSCRIPT_COMPILER = compiler
 ...
 */
export function selectArtifacts(workspace, declaredEntryFiles, packageJson) {
  const present = [], absent = []
  let source = "manifest declaredEntryFiles"
  let declared = declaredEntryFiles ?? []
  if (!declared.length) {
    // The workload manifest has no frozen entry list for this library yet, so
    // take the package's own published entry points and say where they came
    // from.
    source = "package.json exports/main/module"
    declared = [...new Set(entryPointsOf(packageJson))]
  }
  if (!declared.length && existsSync(join(workspace, "dist"))) {
    // An internal comparison workload publishes nothing. Its boundary is
    // whatever its own suite loads out of `dist`, which is a reduced boundary
    // and must be labelled as one rather than read as a whole-library result.
    source = "dist directory; the package declares no published entry point, so this is a reduced boundary"
    declared = readdirSync(join(workspace, "dist")).filter(name => /\.(?:js|cjs|mjs)$/.test(name)).sort().map(name => `dist/${name}`)
  }
  ...
  return { present, absent, source, declared }
}

/** Every file path a package.json publishes through exports, main or module. */
export function entryPointsOf(packageJson) {
  const found = []
  const walk = value => {
    if (typeof value === "string") { if (value.startsWith("./")) found.push(value) }
  ...
  for (const key of ["main", "module", "browser", "unpkg", "jsdelivr"]) {
    ...
    if (typeof value === "string") found.push(value.startsWith("./") ? value : `./${value}`)
  }
  return found.filter(path => /\.(?:js|cjs|mjs)$/.test(path))
}
 ...
 */
export async function discoverAdapter({ library, directory, node = process.execPath, timeoutMs = 300_000, extraLimitations = [], compiler = join(root, "target/release/lilscript") }) {
  ...
  const compilerPinned = existsSync(compiler) ? preserveBinary(compiler) : null
  const environment = sanitizeEnvironment(process.env, { node, scratch, userConfig, globalConfig, compiler: compilerPinned?.path })
  ...
  const DEPENDENCY_LIMIT = { maxBytes: 8 * 1024 * 1024 * 1024, maxEntries: 400_000 }
  // `protected` is what must not move: the original workspace, its installed
  // dependencies and the tools. `scratch` is where the suite is allowed to
  // write, so its changes are reported instead of failing the observation.
  const capture = () => ({
    protectedInputs: {
      inputs: pins(), source: snapshotInputs(original), original: full(original),
      project: snapshotDependencyTree(join(original, "node_modules"), DEPENDENCY_LIMIT),
      npm: snapshotDependencyTree(npmRoot, DEPENDENCY_LIMIT),
    },
    scratch: full(workspace),
  })
  ...
  const receipt = {
    ...
    original, workspace, node: identify(node), compiler: compilerPinned, boundsMs: { suite: timeoutMs },
  ...
  try {
    ...
    receipt.originalTestScript = script ?? null
    const derivation = deriveAdapter(script, { scripts: packageJson.scripts ?? {} })
    receipt.derivation = derivation
    assert(derivation.supported, `original test command cannot be observed: ${derivation.unsupported.join("; ")}`)
    ...
    const artifacts = selectArtifacts(original, library.package?.declaredEntryFiles, packageJson)
    receipt.artifacts = artifacts
    if (artifacts.source !== "manifest declaredEntryFiles") receipt.limitations.push(`Entry files were taken from the ${artifacts.source}; the workload manifest declares none for this library.`)
    assert(artifacts.present.length, `no published entry file exists in the workspace (source: ${artifacts.source}; declared: ${artifacts.declared.join(", ") || "none"})`)
    ...
    before = capture()
    assert.deepEqual(before.scratch.files, before.protectedInputs.original.files, "scratch copy differs from the original workspace")
    save("before.json", before)

    const fixtures = before.protectedInputs.source.files.filter(row => FIXTURE_PATTERN.test(row.path)).map(({ path, sha256, bytes }) => ({ path, sha256, bytes }))
    ...
    const fixturePrefix = "required fixture differs: "
    ...
    // A fixture that changes only after the suite ran was rewritten by the
    // suite itself. The original workspace is a separate copy and is proved
    // unchanged below, so this is recorded as a property of the suite.
    const mutatedBefore = new Set((report.fixtureInputs?.before ?? []).filter(input => !input.matches).map(input => input.expected.path))
    const mutated = report.errors.filter(line => line.startsWith(fixturePrefix)).map(line => line.slice(fixturePrefix.length))
    receipt.mutatedFixtures = [...new Set(mutated.filter(path => !mutatedBefore.has(path)))]
    const unexpected = report.errors.filter(line =>
      line !== "required test inventory missing or ambiguous" && !line.startsWith(untestedPrefix) &&
      !(line.startsWith(fixturePrefix) && receipt.mutatedFixtures.includes(line.slice(fixturePrefix.length))))
    ...
    try {
      ...
      assert.deepEqual(after.protectedInputs, before.protectedInputs, "the original workspace, its dependencies or the tools changed during discovery")
      receipt.inputsStable = true
      const beforeScratch = new Map(before.scratch.files.map(row => [row.path, row.sha256]))
      receipt.workspaceMutations = [
        ...after.scratch.files.filter(row => beforeScratch.get(row.path) !== row.sha256).map(row => row.path),
        ...before.scratch.files.filter(row => !after.scratch.files.some(other => other.path === row.path)).map(row => `${row.path} (removed)`),
      ].sort()
    } catch (error) {
