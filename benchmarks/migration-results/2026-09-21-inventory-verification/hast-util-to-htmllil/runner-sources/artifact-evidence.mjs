...
// Installed code includes nested dependencies and symlink destinations, unlike source snapshots.
export function snapshotDependencyTree(root, { maxBytes = 512 * 1024 * 1024, maxEntries = 50_000 } = {}) {
  for (const value of [maxBytes, maxEntries]) if (!Number.isSafeInteger(value) || value <= 0) throw new Error("dependency snapshot limits must be positive integers")
  root = resolve(root)
  const files = []
  let bytes = 0
  function walk(path, ancestors = new Set()) {
    if (files.length >= maxEntries) throw new Error("dependency snapshot entry limit exceeded")
    const actual = realpathSync(path), info = statSync(path)
    if (ancestors.has(actual)) throw new Error(`dependency symlink cycle: ${path}`)
    const entry = { path: relative(root, path), kind: info.isDirectory() ? "directory" : "file" }
    ...
    if (info.isDirectory()) {
      ...
      if (!info.isFile()) throw new Error(`unsupported dependency input: ${path}`)
      bytes += info.size
      if (bytes > maxBytes) throw new Error("dependency snapshot byte limit exceeded")
      files.push({ ...entry, executable: Boolean(info.mode & 0o111), ...fileIdentity(path) })
    }
  }
  walk(root)
  return { root, files, bytes, sha256: fingerprint(files) }
}
