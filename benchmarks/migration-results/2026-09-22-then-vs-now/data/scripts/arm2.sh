...
[ -n "${CONFIG_DROP:-}" ] && sed -i "/^${CONFIG_DROP}[[:space:]]*=/d" $WS/lilscript.toml
...
printf '{"arm":"%s","port":"%s","route":"%s","patched":"%s","status":%s,"seconds":%s,"config_drop":"%s","compiler_sha256":"%s","sizes":%s}\n' \
  "$ARM" "$PORT" "$ROUTE" "$PATCHED" "$STATUS" "$SECONDS_TAKEN" "${CONFIG_DROP:-}" "$(sha256sum $COMPILER | cut -d' ' -f1)" "$SIZES" > $OUT/$PORT.json
cat $OUT/$PORT.json
