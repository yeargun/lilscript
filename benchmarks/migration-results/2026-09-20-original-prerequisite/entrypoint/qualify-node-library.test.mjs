...
test("qualification entrypoint checks original prerequisite evidence before any producer runs", async t => {
  const base = fileURLToPath(new URL("../../benchmarks/migration-results/2026-09-20-remark-gfm-adapter/existing-dist/", import.meta.url))
  const inventoryPath = fileURLToPath(new URL("../../benchmarks/libraries/remark-gfmlil.required-tests.json", import.meta.url))
  for (const reason of ["report-pin", "declaration", "execution", "valid-prerequisite"]) {
    const f = fixture(t), inventory = json(inventoryPath), discovery = json(join(base, "receipt.json"))
    const report = json(join(base, "node/report.json")), reportPath = join(f.recipe.discovery.directory, "node/report.json")
    if (reason === "execution") report.prerequisites[0].status = 1
    if (reason === "declaration") inventory.prerequisites[0].args = ["run", "test:types"]
    save(reportPath, report)
    inventory.capturedFrom = discovery.report = { path: reportPath, ...fileIdentity(reportPath) }
    if (reason === "report-pin") discovery.report = { ...discovery.report, sha256: "0".repeat(64) }
    const receiptPath = join(f.recipe.discovery.directory, "receipt.json")
    save(receiptPath, discovery)
    Object.assign(f.recipe, {
      workload: "remark-gfmlil", workspace: "/home/azureuser/remark-gfmlil",
      inventoryPath: join(f.root, "remark-gfmlil.required-tests.json"),
      discovery: { ...f.recipe.discovery, sha256: fileIdentity(receiptPath).sha256 },
      // The next gate refuses a valid prerequisite, keeping every producer unreachable.
      caseCount: inventory.requiredCases.length + 1, testFileCount: inventory.testFiles.length,
    })
    save(f.recipe.inventoryPath, inventory)
    f.refreshEntry()
    const output = join(f.root, "refused"), receipt = await qualifyNodeLibrary(f.recipe, [output], f.entryPath)
    t.after(() => rmSync(dirname(receipt.arm), { recursive: true, force: true }))
    assert.equal(receipt.parent?.sha256, f.recipe.defaultParent.sha256, reason)
    const errors = {
      "report-pin": /discovery report identity differs/,
      declaration: /original prerequisite declaration differs/,
      execution: /original prerequisite did not complete successfully/,
      "valid-prerequisite": /22 !== 23/,
    }
    assert.match(receipt.failure.message, errors[reason], reason)
    assert.equal(receipt.passed, false, reason)
    assert.equal(receipt.originalTestBoundaryPassed, false, reason)
    assert.equal(receipt.packageDryRunPassed, false, reason)
    assert.equal(receipt.inputsStable, true, reason)
    ...
    assert.deepEqual(receipt.commands, [], reason)
    assert.equal(existsSync(f.invoked), false, reason)
    assert.equal(existsSync(join(output, "discovery-report.json")), reason === "valid-prerequisite", reason)
    assert.deepEqual(json(join(output, "receipt.json")), receipt)
  }
})
