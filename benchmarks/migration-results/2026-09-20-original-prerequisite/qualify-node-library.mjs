...
import { digest, fileIdentity, fingerprint, snapshotInputs, snapshotDependencyTree as dependencyTree, validateBuild, validateMeasurements, validateTests } from "./artifact-evidence.mjs";
...
export function readQualifiedDiscovery(inventory, discovery, reportPath, npmIdentity, prerequisiteCeiling) {
  const identity = fileIdentity(reportPath);
  for (const pin of [discovery.report, inventory.capturedFrom]) {
    assert(pin, "discovery report identity is required");
    assert.deepEqual(identity, { sha256: pin.sha256, bytes: pin.bytes }, "discovery report identity differs");
  }
  const report = JSON.parse(readFileSync(reportPath, "utf8"));
  assert.equal(report.suiteExecuted, true, "original discovery suite did not execute");
  assert.deepEqual(report.errors, ["required test inventory missing or ambiguous"], "original discovery has additional errors");
  assert.equal(report.evidence.exitCode, 0, "original discovery suite failed");
  assert(report.evidence.cases.length > 0 && report.evidence.cases.every(row => row.status === "pass"), "original discovery cases did not all pass");
  assert.deepEqual(report.evidence.cases.map(row => row.id).sort(), inventory.requiredCases, "original case identities differ");
  assert.equal(inventory.prerequisites.length, 1, "this build contract requires one original prerequisite");
  assert.deepEqual(report.prerequisites.map(row => row.declaration), inventory.prerequisites, "original prerequisite declaration differs");
  const declaration = inventory.prerequisites[0], observed = report.prerequisites[0];
  assert.deepEqual(declaration.executable, npmIdentity, "original prerequisite executable differs");
  assert(Array.isArray(declaration.args) && declaration.args.length === 2 && declaration.args[0] === "run" &&
    textValue(declaration.args[1]) && !declaration.args[1].startsWith("-"), "original prerequisite must name an npm run script without optional flags");
  assert(Number.isSafeInteger(declaration.timeoutMs) && declaration.timeoutMs > 0 && declaration.timeoutMs <= prerequisiteCeiling,
    "original prerequisite exceeds the positive bounded ceiling");
  assert(Array.isArray(declaration.inputs) && declaration.inputs.length > 0, "original prerequisite inputs are required");
  assert(observed.attempted === true && observed.status === 0 && !observed.signal && !observed.error,
    "original prerequisite did not complete successfully");
  assert(observed.supervision?.timedOut === false && observed.supervision.childExitObserved === true &&
    observed.supervision.forcedPipeClosure === false && Number.isSafeInteger(observed.supervision.timeoutMs) &&
    observed.supervision.timeoutMs > 0 && observed.supervision.timeoutMs <= declaration.timeoutMs,
    "original prerequisite supervision is incomplete or unbounded");
  const expected = [declaration.executable, ...declaration.inputs];
  for (const stage of ["before", "after", "final"]) {
    assert(Array.isArray(observed[stage]), `original prerequisite ${stage} identities are missing`);
    assert.deepEqual(observed[stage].map(row => row.expected), expected, `original prerequisite ${stage} identity set differs`);
    for (const row of observed[stage]) {
      assert.equal(row.matches, true, `original prerequisite ${stage} input differs`);
      assert.deepEqual(row.actual, { sha256: row.expected.sha256, bytes: row.expected.bytes }, `original prerequisite ${stage} actual identity differs`);
    }
  }
  return report;
}
...
export async function qualifyNodeLibrary(recipe, argv, entryPath) {
  ...
  try {
    ...
    readQualifiedDiscovery(inventory, discovery, discoveryPaths[2], identify(npm), receipt.boundsMs.prerequisiteCeiling);
