  ...
  if (!skipTests && verdict.trust === "ok") {
    ...
    if (nodeAdapter || vitestAdapter) {
      ...
      const report = nodeAdapter
        ? await runNodeTestEvidence({ ...options, files: [...(adapter.files ?? []), ...(adapter.sharedFixture ? [".lilscript-test-adapter/suite.test.mjs"] : [])], requiredFilePatterns: inventory.entryPatterns, prerequisites: inventory.prerequisites ?? [], nodeArguments: adapter.nodeArguments ?? [] })
