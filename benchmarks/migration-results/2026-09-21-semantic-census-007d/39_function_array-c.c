...
struct ls_array0 { ls_native_object owner; size_t length; size_t capacity; ls_callable0 *items; };
static void ls_array0_acquire(ls_callable0 value) { (void)value; ls_native_retain(value.environment);
}
static void ls_array0_drop(ls_callable0 value) { (void)value; ls_native_release(value.environment);
}
...
ls_callable0 *items = realloc(array->items, capacity * sizeof *items);
if (!items) ls_native_resource_failure();
array->items = items; array->capacity = capacity;
}
static int32_t ls_array0_push_owned(ls_array0 *array, ls_callable0 value) {
...
static int32_t ls_array0_push(ls_array0 *array, ls_callable0 value) {
...
static ls_callable0 ls_array0_get(ls_array0 *array, int32_t index) {
if (index < 0 || (size_t)index >= array->length) { ls_native_undefined_element(); return array->items[0]; }
return array->items[index];
}
static void ls_array0_set(ls_array0 *array, int32_t index, ls_callable0 value) {
...
static ls_callable0 ls_array0_pop(ls_array0 *array) {
...
for (size_t low = 0, high = array->length; low + 1 < high; low++, high--) { ls_callable0 value = array->items[low]; array->items[low] = array->items[high - 1]; array->items[high - 1] = value; }
return array;
}
static ls_array0 *ls_array0_fill(ls_array0 *array, ls_callable0 value) {
...
static int32_t ls_array0_index_of(ls_array0 *array, ls_callable0 right) {
for (size_t index = 0; index < array->length; index++) { ls_callable0 left = array->items[index]; if (left.identity == right.identity) return (int32_t)index; }
return -1;
}
static bool ls_array0_includes(ls_array0 *array, ls_callable0 right, int32_t start) {
size_t length = array->length;
size_t from = start >= 0 ? (size_t)start : ls_array_relative(start, length);
for (size_t index = from; index < length; index++) { ls_callable0 left = array->items[index]; if (left.identity == right.identity) return true; }
return false;
}
...
ls_array0_push(ls_v4,(ls_callable0){ls_adapter1,NULL,UINT64_C(2)});
ls_array0_push(ls_v4,(ls_callable0){ls_adapter2,NULL,UINT64_C(3)});
ls_array0_copy(&(ls_c2),ls_v4);
...
ls_test29: ;
ls_v7 = ls_c6;
ls_array0_copy(&(ls_v8),ls_c2);
ls_v9 = (int32_t)ls_v8->length;
ls_v10 = ls_v7 < ls_v9;
ls_array0_clear(&ls_v8);
if (!ls_v10) goto ls_end29;
ls_array0_copy(&(ls_v11),ls_c2);
ls_v12 = ls_c6;
ls_callable0_copy(&(ls_v13),ls_array0_get(ls_v11,ls_v12));
ls_callable0_copy(&(ls_c7),ls_v13);
ls_callable0_copy(&(ls_v14),ls_c7);
ls_v15 = ls_c3;
