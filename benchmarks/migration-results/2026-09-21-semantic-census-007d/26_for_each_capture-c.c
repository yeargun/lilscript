...
static inline void ls_callable0_call(ls_callable0 value,int32_t ls_p0) {
ls_native_retain(value.environment);
...
static int32_t ls_c0;
static bool ls_ready0;
static int32_t *ls_g0(void) {
if (!ls_ready0) ls_native_unbound();
return &ls_c0;
}
static void ls_init0(void);
static void ls_fn1(void *ls_env,int32_t ls_c2);
...
ls_v3 = ls_from_u32(UINT32_C(5));
ls_v4 = ls_from_u32(UINT32_C(7));
...
ls_array0_copy(&(ls_v6),ls_c1);
ls_callable0_take(&(ls_v7),ls_closure1());
{
ls_array0 *ls_src = ls_v6;
size_t ls_len = ls_src->length;
for (size_t ls_k = 0; ls_k < ls_len; ls_k++) {
if (ls_k >= ls_src->length) continue;
ls_v7.code(ls_v7.environment,ls_src->items[ls_k]);
}
}
ls_v9 = ls_c0;
printf("%ld\n",(long)ls_v9);
