...
struct ls_array0 { ls_native_object owner; size_t length; size_t capacity; double *items; };
static void ls_array0_acquire(double value) { (void)value; }
static void ls_array0_drop(double value) { (void)value; }
...
double *items = realloc(array->items, capacity * sizeof *items);
if (!items) ls_native_resource_failure();
array->items = items; array->capacity = capacity;
}
static int32_t ls_array0_push_owned(ls_array0 *array, double value) {
...
static int32_t ls_array0_push(ls_array0 *array, double value) {
ls_array0_acquire(value);
return ls_array0_push_owned(array, value);
}
static void ls_array0_hole(ls_array0 *array) { (void)array; ls_native_undefined_element(); }
static double ls_array0_get(ls_array0 *array, int32_t index) {
if (index < 0 || (size_t)index >= array->length) { ls_native_undefined_element(); return array->items[0]; }
return array->items[index];
}
static void ls_array0_set(ls_array0 *array, int32_t index, double value) {
...
static double ls_array0_pop(ls_array0 *array) {
...
for (size_t low = 0, high = array->length; low + 1 < high; low++, high--) { double value = array->items[low]; array->items[low] = array->items[high - 1]; array->items[high - 1] = value; }
return array;
}
static ls_array0 *ls_array0_fill(ls_array0 *array, double value) {
...
static int32_t ls_array0_index_of(ls_array0 *array, double right) {
for (size_t index = 0; index < array->length; index++) { double left = array->items[index]; if (left == right) return (int32_t)index; }
return -1;
}
static bool ls_array0_includes(ls_array0 *array, double right, int32_t start) {
size_t length = array->length;
size_t from = start >= 0 ? (size_t)start : ls_array_relative(start, length);
for (size_t index = from; index < length; index++) { double left = array->items[index]; if (left == right || (left != left && right != right)) return true; }
return false;
}
...
int32_t ls_v42;
int32_t ls_v43;
ls_v0 = ls_f64_bits(UINT64_C(0));
ls_v1 = ls_f64_bits(UINT64_C(4598175219545276416));
ls_v2 = ls_f64_bits(UINT64_C(4607182418800017408));
ls_array0_take(&(ls_v3),ls_array0_new(3));
ls_array0_push(ls_v3,ls_v0);
ls_array0_push(ls_v3,ls_v1);
ls_array0_push(ls_v3,ls_v2);
ls_array0_copy(&(ls_c0),ls_v3);
...
ls_test27: ;
ls_v6 = ls_c3;
ls_array0_copy(&(ls_v7),ls_c0);
ls_v8 = (int32_t)ls_v7->length;
ls_v9 = ls_v6 < ls_v8;
ls_array0_clear(&ls_v7);
if (!ls_v9) goto ls_end27;
ls_array0_copy(&(ls_v10),ls_c0);
ls_v11 = ls_c3;
ls_v12 = ls_array0_get(ls_v10,ls_v11);
ls_v13 = ls_f64_bits(UINT64_C(4598175219545276416));
ls_v14 = ls_v12 >= ls_v13;
if (ls_v14) {
ls_v15 = ls_c1;
ls_v16 = ls_from_u32(UINT32_C(1));
ls_v17 = ls_from_u32((uint32_t)((uint32_t)ls_v15 + (uint32_t)ls_v16));
ls_c1 = ls_v17;
}
...
goto ls_test27;
ls_end27: ;
ls_v21 = ls_f64_bits(UINT64_C(4598175219545276416));
ls_v22 = ls_f64_bits(UINT64_C(4598175219545276416));
ls_v23 = ls_f64_bits(UINT64_C(4598175219545276416));
ls_v24 = ls_f64_bits(UINT64_C(4598175219545276416));
ls_v25 = ls_f64_bits(UINT64_C(4602678819172646912));
ls_array0_take(&(ls_v26),ls_array0_new(5));
ls_array0_push(ls_v26,ls_v21);
ls_array0_push(ls_v26,ls_v22);
ls_array0_push(ls_v26,ls_v23);
ls_array0_push(ls_v26,ls_v24);
ls_array0_push(ls_v26,ls_v25);
ls_array0_copy(&(ls_c2),ls_v26);
ls_v27 = ls_from_u32(UINT32_C(0));
ls_c4 = ls_v27;
ls_test56: ;
ls_v28 = ls_c4;
ls_array0_copy(&(ls_v29),ls_c2);
ls_v30 = (int32_t)ls_v29->length;
ls_v31 = ls_v28 < ls_v30;
ls_array0_clear(&ls_v29);
if (!ls_v31) goto ls_end56;
ls_array0_copy(&(ls_v32),ls_c2);
ls_v33 = ls_c4;
ls_v34 = ls_array0_get(ls_v32,ls_v33);
ls_v35 = ls_f64_bits(UINT64_C(4598175219545276416));
ls_v36 = ls_v34 == ls_v35;
if (ls_v36) {
ls_v37 = ls_c1;
...
ls_array0_clear(&ls_v32);
ls_update56: ;
ls_v40 = ls_c4;
ls_v41 = ls_from_u32(UINT32_C(1));
ls_v42 = ls_from_u32((uint32_t)((uint32_t)ls_v40 + (uint32_t)ls_v41));
ls_c4 = ls_v42;
goto ls_test56;
ls_end56: ;
ls_v43 = ls_c1;
printf("%ld\n",(long)ls_v43);
ls_array0_clear(&ls_c2);
