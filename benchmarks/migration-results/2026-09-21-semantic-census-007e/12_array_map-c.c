...
static int32_t ls_fn1(void *ls_env,int32_t ls_c2);
...
ls_callable0 ls_v6 = {0};
ls_array0 * ls_v7 = NULL;
ls_array0 * ls_v8 = NULL;
ls_callable2 ls_v9 = {0};
int32_t ls_v10;
int32_t ls_v11;
int32_t ls_v12;
...
ls_callable0_take(&(ls_v6),ls_closure1());
{
ls_array0 *ls_src = ls_v5;
...
ls_array0_push_owned(ls_out,ls_v6.code(ls_v6.environment,ls_src->items[ls_k]));
}
ls_array0_take(&(ls_v7),ls_out);
}
ls_array0_copy(&(ls_c3),ls_v7);
ls_array0_copy(&(ls_v8),ls_c3);
ls_callable2_take(&(ls_v9),ls_closure2());
ls_v10 = ls_from_u32(UINT32_C(0));
{
int32_t ls_acc = ls_v10;
{
ls_array0 *ls_src = ls_v8;
size_t ls_len = ls_src->length;
for (size_t ls_k = 0; ls_k < ls_len; ls_k++) {
if (ls_k >= ls_src->length) continue;
ls_acc = ls_v9.code(ls_v9.environment,ls_acc,ls_src->items[ls_k]);
}
}
ls_v11 = ls_acc;
}
