...
static inline bool ls_callable0_call(ls_callable0 value,int32_t ls_p0) {
ls_native_retain(value.environment);
...
static bool ls_fn1(void *ls_env,int32_t ls_c2);
static int32_t ls_fn2(void *ls_env,int32_t ls_c4,int32_t ls_c5);
static ls_callable0 ls_closure1(void) {
return (ls_callable0){ls_fn1,NULL,ls_native_fresh_identity()};
}
static ls_callable2 ls_closure2(void) {
return (ls_callable2){ls_fn2,NULL,ls_native_fresh_identity()};
}
static void ls_init0(void) {
...
ls_callable0 ls_v8 = {0};
ls_array0 * ls_v9 = NULL;
ls_array0 * ls_v10 = NULL;
ls_callable2 ls_v11 = {0};
int32_t ls_v12;
int32_t ls_v13;
int32_t ls_v14;
...
ls_v5 = ls_from_u32(UINT32_C(6));
ls_array0_take(&(ls_v6),ls_array0_new(6));
ls_array0_push(ls_v6,ls_v0);
ls_array0_push(ls_v6,ls_v1);
ls_array0_push(ls_v6,ls_v2);
ls_array0_push(ls_v6,ls_v3);
ls_array0_push(ls_v6,ls_v4);
ls_array0_push(ls_v6,ls_v5);
ls_array0_copy(&(ls_c0),ls_v6);
ls_array0_copy(&(ls_v7),ls_c0);
ls_callable0_take(&(ls_v8),ls_closure1());
{
...
if (ls_v8.code(ls_v8.environment,ls_item)) ls_array0_push(ls_out, ls_item);
}
ls_array0_take(&(ls_v9),ls_out);
}
ls_array0_copy(&(ls_c3),ls_v9);
ls_array0_copy(&(ls_v10),ls_c3);
ls_callable2_take(&(ls_v11),ls_closure2());
ls_v12 = ls_from_u32(UINT32_C(0));
{
int32_t ls_acc = ls_v12;
{
ls_array0 *ls_src = ls_v10;
size_t ls_len = ls_src->length;
for (size_t ls_k = 0; ls_k < ls_len; ls_k++) {
if (ls_k >= ls_src->length) continue;
ls_acc = ls_v11.code(ls_v11.environment,ls_acc,ls_src->items[ls_k]);
}
}
ls_v13 = ls_acc;
}
