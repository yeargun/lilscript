...
function prerequisiteInputs(cwd, prerequisite) {
  return [prerequisite.executable, ...prerequisite.inputs].map(expected => {
...
export async function runNodeTestEvidence({ cwd, files, artifactPaths, requiredCases, requiredTestFiles, requiredFilePatterns, requiredFixtures = [], prerequisites = [], directory, node = process.execPath, nodeArguments = [], timeoutMs = 2700000 }) {
  ...
  const suiteExecuted = prerequisiteErrors.length === 0
  ...
  const errors = [...prerequisiteErrors, ...validateTests(evidence)]
  if (!suiteExecuted) errors.push("Node suite was not executed because a prerequisite did not pass")
  ...
  const report = {
    ...
    runtime, toolInputs,
    ...(prerequisites.length ? { prerequisites: prerequisiteReports, prerequisiteEnvironment: { PATH: prerequisiteEnvironment.PATH }, suiteExecuted } : {}),
