...
async function runPort(port, { compiler, pinnedCodec, timeoutS, skipTests, logDir, armDirectory }) {
  ...
  if (verdict.trust === "ok") {
    ...
    const measurement = await measure(paths, pinnedCodec, timeoutS * 1000)
  ...
  if (!skipTests && verdict.trust === "ok") {
    ...
    if (nodeAdapter || vitestAdapter) {
      ...
      const options = { cwd, artifactPaths: adapter.artifacts, requiredCases: inventory.requiredCases, requiredTestFiles: inventory.testFiles, requiredFixtures: inventory.fixtures, directory: join(armDirectory, "tests", port), timeoutMs: timeoutS * 1000 }
      const report = nodeAdapter
        ? await runNodeTestEvidence({ ...options, files: [...(adapter.files ?? []), ...(adapter.sharedFixture ? [".lilscript-test-adapter/suite.test.mjs"] : [])], requiredFilePatterns: inventory.entryPatterns, nodeArguments: adapter.nodeArguments ?? [] })
...
async function record() {
  ...
  if (!Number.isSafeInteger(timeoutS * 1000) || timeoutS <= 0) die("--timeout must specify a finite positive number of milliseconds in seconds")
  ...
  for (const port of ports) {
    const row = await runPort(port, { compiler: armCompiler, pinnedCodec: armCodec, timeoutS, skipTests, logDir, armDirectory: out })
