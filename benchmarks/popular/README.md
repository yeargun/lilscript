...
| LilScript | LilScript port under size-first / Brotli cost model |
...
gates. Adapter bytes are always counted. Raw plus the configured transport codec
must be no larger than either npm/Vite 8 or public-API-preserving Closure
ADVANCED; gzip and Brotli are both reported because choosing one exact objective
can trade a few bytes in the other. C/native execution is extra
...
processes with identical workloads and checksums.
