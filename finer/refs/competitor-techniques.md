...
| Multi-use constant inlining: `var x="lit"` read N times → the literal at every read | `inline_identifier_reference` (`OXC/peephole/inline.rs:104-153`): constant literal values of `const` bindings, no size rule, gated on the value being a cheap literal | `TERSER/compress/inline.js:303-313`: replace iff `replace_size <= name_length + overhead`, `overhead = (name_length + 2 + value_size) / (references − assignments)` when `unused` is on (the declaration's amortised cost); a 6-byte string never replaces a 2-byte name used 900 times | `propagate_single_assignment_globals` (`LS/optimizer.rs:3345-3410`): every load of a single-store, non-exported global becomes the constant, no cost rule, before the search | **PRESENT, and right under Brotli**: 046 measured Terser's shape on katexlil's 1960 sites at −8207 raw / **+129 Brotli**; the raw rule is not adopted while the objective is Brotli (a raw-objective port would want it) |

### D.2 Global singletons, the `arguments` object, and null normalization (harvest 2026-09-02, 048)

| Technique | Closure | terser | oxc | LilScript | Status |
|---|---|---|---|---|---|
| Dissolve a **global singleton aggregate** (one allocation at module init, identity never reaching a host) into scalars — the shape upstream `cn` hand-writes as a closure over `let`/`const` state | `InlineAndCollapseProperties` collapses global *qualified names* set once (`GlobalNamespace.Name#canCollapseOrInline`, `GlobalNamespace.java:1844-1920`: refuses externs, accessors, `@nocollapse`, `delete`, spread-followed and conditional definitions); `InlineObjectLiterals` (`:28-31, 119-131, 146-230`) is local-only and literal-only. A `new` instance's fields are not namespace names: never collapsed | `hoist_props` (`TERSER/compress/index.js:883-948`): object *literal* only, never escaped (`reduce-vars.js:228-270`), no `direct_access`; `new` instances excluded | ABSENT (§D row above) | `scalar_replace_linear_classes` (`LS/optimizer.rs:7149, 7186-7219`) requires `EscapeState::LocalOnly`; a module-global instance stored into another instance's field is emitted as a positional handle (`aggregate_layout` default), and every hot read becomes `h[NN]` (cnlil: arb 1.28x, workset 1.33x, +2000 raw of init and defaults) | **ABSENT everywhere; the port spells module state (048)**. Compiler lead: a module-init allocation whose identity never reaches export/host/`JsValue` is dissolvable |
| Pass the **`arguments` object whole** from a rest-style wrapper to its slow path | — | — | rewrites the copy-loop *shape* only (`try_rewrite_arguments_copy_loop`, §A row) | `JsCallingConvention::StaticRest` aliases the callback's `args` to `arguments` (`LS/codegen_ir_js.rs:7347-7348, 2426`); a whole-`args` use is spelled `f(arguments)`, which V8 materializes on every call, predicted hits included (cnlil component:loop 1.48x → 1.09x with `[a,b,c]` on the miss path only) | **Port item** today (read by index, copy on the miss path); a compiler lead: spell a whole-`args` use on a cold path as a copy |
| **Null normalization** of an absent record/map read: `x=E??null` then `null!=x` | — | — | — | `IndexGet` on a `Record<T>` emits `{indexed}??null` (`LS/codegen_ir_js.rs:12912-12920`); `fold_null_normalized_nullable_tests` (`LS/js_peephole/folds/boolean.rs`, 048, branch `finer/048-nullish-fold`) drops it when every other same-scope use is guarded or killed. Perf: nil — one process per variant on Node 20 and 22 puts `null!=x`, `x!==void 0`, `??null` within 5% of each other; the earlier 8% was a measurement order artifact. What costs 15% on the hit path is **module-scope state**: V8 reaches a module binding through a cell (`LdaModuleVariable`), a closure binding through one context slot | **PARTIAL** (fold on a branch; the peephole itself never runs at level 8 with `candidate_search = "off"`: `peephole_calls=0`, `cleanup_entered=0`) |
...
| 27 | Sequence parenthesisation by parent precedence | **PRESENT per fold, checked at admission (044)** | Terser, Oxc and Closure decide it at print time: `TERSER/output.js` `PARENS(AST_Sequence, …)` wraps a sequence under an `AST_Conditional`, `AST_Arrow`, `AST_Call`, unary or binary parent, so `if`-to-conditional folds (`TERSER/compress/index.js:1156-1208`) build sequences freely; Oxc's `print_expr` and Closure's `CodeGenerator.addExpr` wrap by precedence. LilScript's folds are the printer, so each fold that re-spells a sub-expression carries the rule: `render_logical_operand` and, since 044, `conditional_arm_text` (`LS/js_peephole/folds/boolean.rs`), with `validate_conditional_arm_sequences` (`LS/js_peephole/mod.rs`) refusing `?a,b:c` at admission. Sibling of row 23 (adjacency at the splice, 040). |
