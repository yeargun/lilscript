// Harness of the source test (first = 0): proxies log every read of input and write to the destination.
(() => {
  const events = [];
  globalThis.input = new Proxy({ first: 0, second: 7 }, { get(object, key) { events.push('get:' + key); return Reflect.get(object, key); } });
  const target = {};
