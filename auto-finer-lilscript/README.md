...
| [021](021-reflective-ffi-predicts-loss/README.md) | Why are the receivers dynamic in the first place? | **The losing ports are JavaScript transliterated through a host FFI.** Reflective `JS.get`/`set`/`call`/`method` per kloc orders the scoreboard: markedlil **0.8 → −579 WIN**, micromarklil **178 → +4154**. This is the answer, and it reorders every other recommendation |
...
| [018](018-mobx-admission-regression/README.md) | mobxlil is +3577 with a frozen source — is that a compiler regression too? | **Yes, but smaller than it looked: `src/compiler.rs` across `42c1ad0..edbdf3a`, −7546 raw and only −253 Brotli.** `class` emission is lost to prototype tables, which compress well. Seven mechanisms falsified. 59s repro |
