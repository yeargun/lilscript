...
| [036](036-the-fix-exists-and-starves/README.md) | Do we need to build scope-aware shadowing? | **No — it exists and starves.** `precise_cross_scope_shadowing` is implemented, registered as a scored family, and its own comment describes 035's defect exactly. `--explain` on micromarklil: **46 of 47 families starved**, `work-budget-exhausted`, terminal probe limit hit. A scored family that never gets measured is indistinguishable from a missing feature |
