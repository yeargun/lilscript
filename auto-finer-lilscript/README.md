...
| [037](037-react-markdown-miscompile/README.md) | Why is react-markdownlil 14166 behind? | **Its config sets `terminal_codec_probe_limit = 0`**, which also skips the canonical rewrite — 726 unmerged `;var` runs. Removing it is **−3364 Brotli** and **breaks the port**: the class rewrite drops `Xa=VFile.prototype` and the module throws on import. `identifier_is_read_after` false-negative, a second hole in the machinery 033 fixed |
| [036](036-the-fix-exists-and-starves/README.md) | Do we need to build scope-aware shadowing? | **No, and we do not want it.** `precise_cross_scope_shadowing` already exists and starves — but forcing it on gives module bindings their one-char names exactly as 035 predicted and costs **+432 Brotli**, because those names come out of the locals' pool. 035's premise falsified: the 62-of-63 two-char naming is the *better* trade. Every budget knob was byte-identical for the same reason |
| [035](035-where-the-compiler-headroom-is/README.md) | What is left for the compiler, measured? | **Not the printer** — a pure reprint of our output saves 0 bytes. It is **name allocation**: we give **62 of 63 top-level bindings two-char names** where Terser gives 53 of 58 one-char ones, because module and local names come from **disjoint pools** while Terser lets locals shadow. Terser extracts **−884 / −856 / −1136** from finished micromark / mobx / jquery artifacts. `local_name_reserve` swept and already optimal |
| [034](034-escaped-non-ascii/README.md) | Why does a *reprint* of our output save 7 KB? | **The bundler escaped 2304 non-ASCII characters** into `\uXXXX` — six ASCII bytes where the literal is two or three UTF-8 ones. Our own output has 48 bytes of slack; the bundle has 7021. `charset: "utf8"` fixes it: **−27689 raw and −1093 Brotli** across micromarklil and rehypelil. Fourth defect living between compiler and artifact |
| [033](033-member-bodies-are-scopes/README.md) | Why could the resolver not find that binding? | **A member body that is not recognised is not a scope**, so its `var`s leak to the module and poison the name. Accessors, `static`, generators, computed names and **keyword-named members** (`delete(e){...}`) were all unrecognised. Fixed: **mobxlil emits 018's ten classes at last**, −6786 raw / −235 Brotli, no other port moves |
| [032](032-export-resolver-false-negative/README.md) | Why does mobxlil still emit 0 classes? | **Our export resolver cannot find a binding node runs fine.** The pipeline turns mobx's shipped artifact into 10 classes and **−769 Brotli**; admission refuses it for an "unresolved export binding" that `node --check`, esbuild, and an actual `import` all resolve. Second admission false-negative in two hypotheses, both refusing the class rewrite |
| [031](031-admission-blocks-the-class-rewrite/README.md) | Why does the canonical peephole never land? | **Admission rejects it for having a `constructor`** — the class rewrite's own keyword, counted as a new static property, at a gate 018 never counted. Exempting it globally measures **+826 worse** (it changes the whole portfolio); exempting it only where an already-won artifact is re-checked is **−194 Brotli** on micromarklil and −29 on markedlil |
