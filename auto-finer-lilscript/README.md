...
| [026](026-the-missing-classes/README.md) | Where did 018's ten `class` declarations go? | **Generated, then corrupted.** `fold_unread_prototype_aliases` deletes seven `g=X.prototype` assignments to a reused temp that is read 224 times; the artifact stops resolving and the *syntax validator* discards it. Every counter 018 added sits at or after admission, so it saw nothing |
